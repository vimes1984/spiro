...
angular.module('spiroApp')
  .controller('SpiroFourCtrl', function ($scope) {
  ...
  //Create elements .arc(x, y, radius, startAngle, endAngle, counterClockwise);
  	...
  	$scope.numberoflines = 100;
  	$scope.beziercurveincX = 0.01;
  	$scope.beziercurveincY = 0.002;
	$scope.redrawit = function redrawit(){
    ...
    $scope.Ymove = Math.floor((Math.random() * 690) - 1);
		  ...
		  for (var i = 0, l = $scope.numberoflines; i < l; i++) {
    ...
    $scope.middlesetY = 350;
    $scope.middlesetX = 350;
    ...
    $scope.updown = false;
  	$scope.draw = function draw(e){
       ...
       // $scope.canvascanvasContext.beginPath();
        ...
        if($scope.updown == true){
            $scope.Ymove += $scope.beziercurveincY;
            if($scope.Ymove > 680){
              $scope.updown=false;
            }
        ...
        if($scope.updown == true){
            $scope.Xmove += $scope.beziercurveincX;
            if($scope.Xmove > 680){
              $scope.updown=false;
            }
         }else if($scope.updown == false){
     ...
     $scope.canvascanvasContext.moveTo($scope.randstarty,$scope.randstartx);
      ...
      $scope.canvascanvasContext.bezierCurveTo($scope.Xmove,$scope.Xmove + Math.PI*2,$scope.Ymove,$scope.Xmove,$scope.middlesetY,$scope.middlesetX);
